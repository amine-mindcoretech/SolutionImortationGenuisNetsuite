...
const contactModel = {
  async createTable() {
    const createTableQuery = `
      CREATE TABLE IF NOT EXISTS contactgenius_import (
        ...
        parent VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci,
    ...
    try {
      ...
      const [columns] = await connection.query("SHOW COLUMNS FROM contactgenius_import");
      const columnNames = columns.map(col => col.Field);

      if (!columnNames.includes('isInactive')) {
        await connection.query("ALTER TABLE contactgenius_import ADD isInactive CHAR(1) DEFAULT 'F'");
        console.log("Added isInactive column to contactgenius_import");
      }
      if (!columnNames.includes('emailTransactions')) {
        await connection.query("ALTER TABLE contactgenius_import ADD emailTransactions CHAR(1) DEFAULT 'F'");
        console.log("Added emailTransactions column to contactgenius_import");
      }
      if (!columnNames.includes('faxTransactions')) {
        await connection.query("ALTER TABLE contactgenius_import ADD faxTransactions CHAR(1) DEFAULT 'F'");
        console.log("Added faxTransactions column to contactgenius_import");
      }
      if (!columnNames.includes('status')) {
        await connection.query("ALTER TABLE contactgenius_import ADD status VARCHAR(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT 'CUSTOMER-Actif'");
        console.log("Added status column to contactgenius_import");
      }
      if (!columnNames.includes('numberformat')) {
        await connection.query("ALTER TABLE contactgenius_import ADD numberformat VARCHAR(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT '1 000 000,00'");
        console.log("Added numberformat column to contactgenius_import");
      }
      if (!columnNames.includes('parent')) {
        await connection.query("ALTER TABLE contactgenius_import ADD parent VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci");
        console.log("Added parent column to contactgenius_import");
      }

      connection.release();
      return { success: true, message: 'Table contactgenius_import created or updated successfully' };
    } catch (error) {
  ...
  async createTempTable() {
    const createTempTableQuery = `
      CREATE TABLE temp_contact_genius (
    ...
    try {
      const connection = await pool.getConnection();
      await connection.query('DROP TABLE IF EXISTS temp_contact_genius');
  ...
  async fetchGeniusData() {
    const geniusQuery = `
      SELECT 
        ...
        c.Email,
        c.Fac_A,
        COALESCE(p.C_No, 'unknown') AS parent -- Use only C_No of parent, default to 'unknown' if null
      FROM Cli c
      LEFT JOIN Cli p ON c.Fac_A = p.C_No
      WHERE 
        c.Actif = 'O' 
        AND c.IsNationalAccount = 0
        AND c.Fac_A <> c.C_No
        AND (c.Pays = 'CANADA' OR c.Pays = 'USA')
        AND c.Adresse <> ''
        AND c.Modeexp NOT IN ('PPC', 'COD', 'COLLECT')
    `;
    try {
  ...
  async syncData() {
    try {
      ...
      await connection.query('TRUNCATE TABLE temp_contact_genius');
      const insertTempQuery = `
        INSERT INTO temp_contact_genius (
          ...
          Noteexp, Notefac, firstName, middleName, lastName, email, parent
        ) VALUES ?
      `;
      const tempValues = geniusData.map(row => [
        ...
        row.Watt, row.Groupe, row.Notecom, row.Noteexp, row.Notefac, row.firstName, row.middleName, row.lastName, row.email, row.parent
      ]);
      console.log(`Inserting ${tempValues.length} rows into temp_contact_genius`);
      if (tempValues.length > 0) {
        await connection.query(insertTempQuery, [tempValues]);
      }

      // Ensure all rows are inserted using C_No as entityid to avoid duplicates
      const transformQuery = `
        INSERT INTO contactgenius_import (
          ...
          custentity_mc_notefac, isInactive, emailTransactions, faxTransactions, status, numberformat, parent
        )
        SELECT 
          Cli.Nom AS entityid,  -- Use C_No as unique identifier
          Cli.Nom AS companyName,
          Cli.Adresse AS address1_line1,
          Cli.CTR_Address2 AS address1_line2,
          Cli.Ville AS address1_city,
          COALESCE(ppm.provincenetsuite, '') AS address1_state,
          COALESCE(ppm.paysnetsuite, '') AS address1_countrys,
          ...
          Cli.middleName,
          Cli.lastName,
          COALESCE(tm.terms_netsuite, '') AS terms,
          COALESCE(cm.currency_netsuite, '') AS currency,
          COALESCE(tpm.netsuite_transport, '') AS custentitycustentity_mc_transport,
          CASE
            WHEN Cli.langue = 'FRENCH' THEN 'French (Canada)'
            WHEN Cli.langue = 'ENGLISH' THEN 'English (U.S.)'
            ELSE ''
          END AS language,
          COALESCE(incom.incoterm_netsuite, '') AS custentity_mc_incoterm_client,
          Cli.email AS email,
          CAST(Cli.datecree AS DATE) AS startdate,
          Cli.Classe AS category,
          Cli.CTR_Link AS externalid,
          COALESCE(modepm.internal_id_netsuite, '') AS custentity_gs_pay_mth,
          ...
          'F' AS isPerson,
          '2' AS subsidiary,
          COALESCE(tmv.netsuite_territoire, '') AS custentity_mct_terri_vente,
          COALESCE(vm.netsuite_salerep, '') AS salesrep,
          Cli.Watt AS comments,
          COALESCE(taxe.netsuite_code, '') AS taxitem,
          ...
          Cli.parent AS parent
        FROM temp_contact_genius Cli
        LEFT JOIN terms_mapping tm ON Cli.Term = tm.terms_genius
        LEFT JOIN currency_mapping cm ON Cli.Devise = cm.currency_genius
        LEFT JOIN transport_mapping tpm ON Cli.Transport = tpm.genius_transport
        LEFT JOIN incoterm_mapping incom ON Cli.Modeexp = incom.incoterm_genius
      ...
      const [transformedData] = await connection.query(`
        SELECT 
          ...
          parent,
          last_updated
        FROM contactgenius_import
      `);

      const [existingRecords] = await connection.query('SELECT entityid FROM contactgenius_import');
      const existingIds = new Set(existingRecords.map(row => row.entityid));
      const newIds = new Set(geniusData.map(row => row.C_No)); // Use C_No for uniqueness

      const idsToDelete = [...existingIds].filter(id => !newIds.has(id));
      if (idsToDelete.length > 0) {
        await connection.query('DELETE FROM contactgenius_import WHERE entityid IN (?)', [idsToDelete]);
      }
  ...
  async dropTable() {
    try {
      const connection = await pool.getConnection();
      await connection.query('DROP TABLE IF EXISTS contactgenius_import');
      await connection.query('DROP TABLE IF EXISTS temp_contact_genius');
      connection.release();
