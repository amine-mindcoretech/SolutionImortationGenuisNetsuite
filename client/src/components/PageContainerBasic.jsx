...
function CustomToolbar({ setColumns, setContacts, filteredContacts, setFilteredContacts }) {
  ...
  const handleSaveToFile = async () => {
    const fileName = 'contacts_updated.xlsx';
    try {
      ...
      const sheetData = filteredContacts.map((row) =>
  ...
  const handleExportCSV = async () => {
    ...
    const rowsData = filteredContacts
      .map((row) =>
    ...
    try {
      if ('showSaveFilePicker' in window) {
        const handle = await window.showSaveFilePicker({
          suggestedName: 'contacts_utf8.csv',
          types: [
            {
  ...
  const handleExportHeaderAndFirstRow = () => {
    const visibleColumns = apiRef.current.getVisibleColumns();
    const firstTenRows = filteredContacts.slice(0, 10);
    ...
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'contacts_utf8_first_10_rows.csv';
    link.click();
  };
  ...
  const handleSaveToMySQL = async () => {
    ...
    try {
      ...
      const dataToSave = filteredContacts.map((row, index) => {
        const normalizedRow = {};
  ...
  const handleSyncData = async () => {
    try {
      await axios.delete('http://localhost:5000/api/contact/drop');
      const response = await axios.post('http://localhost:5000/api/contact/sync');
  ...
  const handleSplitContact = () => {
    ...
    setContacts((prevContacts) =>
      prevContacts.map((contact) => {
        const contactField = contact.contact || '';
        let firstName = '', middleName = '', lastName = '';

        if (!contactField || typeof contactField !== 'string' || contactField.toLowerCase().includes('online buy')) {
          return { ...contact, firstName, middleName, lastName };
        }

        const emailMatch = contactField.match(/<(.+@.+)>/);
        let namePart = emailMatch ? contactField.split('<')[0].trim() : contactField.trim();
        ...
        } else if (!emailMatch && contactField.includes('@')) {
          const emailParts = contactField.split('@')[0].split(/[_.-]/);
          if (emailParts.length >= 2) {
            firstName = emailParts[0];
            lastName = emailParts.slice(1).join('');
          } else {
...
export default function PageContainerBasicContact() {
    ...
    if (filterColumn && filterValues) {
      ...
      if (valuesToFilter.length > 0) {
        filteredData = filteredData.filter((contact) => {
          const value = contact[filterColumn];
  ...
  }, [contacts, searchTerm, filterColumn, filterValues, filterCondition]);
  ...
  const handleUpdateValues = () => {
    ...
    setContacts((prevContacts) =>
      prevContacts.map((contact) => {
        const currentValue = contact[selectedColumn];
        if (existingValue === '' && (currentValue === '' || currentValue === null || currentValue === undefined)) {
          return { ...contact, [selectedColumn]: newValue };
        }
  ...
  const handleExtractDate = () => {
    ...
    setContacts((prevContacts) =>
      prevContacts.map((contact) => {
        const value = contact[selectedColumn];
        if (value && typeof value === 'string' && /\d{4}-\d{2}-\d{2}/.test(value)) {
          const datePart = value.split(' ')[0];
          return { ...contact, [selectedColumn]: datePart };
        }
        return contact;
      })
    );
  ...
  return (
    <div style={{ width: '100%', padding: '20px' }}>
      <Typography variant="h4" sx={{ marginBottom: '20px' }}>
        Outil de Modification Contacts
      </Typography>
